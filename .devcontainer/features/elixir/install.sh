...
otp_version=${OTPVERSION:-"latest"}
elixir_version=${VERSION:-"latest"}
force=false
...
main() {
  ...
  root_dir="$_REMOTE_USER_HOME/.elixir-install"
...
ACTUAL_OTP_VERSION=$(find $_REMOTE_USER_HOME/.elixir-install/installs/otp -type d -maxdepth 1 -mindepth 1 | sort | tail -1 | xargs basename)
ACTUAL_ELIXIR_VERSION=$(find $_REMOTE_USER_HOME/.elixir-install/installs/elixir -type d -maxdepth 1 -mindepth 1 | sort | tail -1 | xargs basename)

# Create PATH entries
mkdir -p /etc/profile.d
cat << EOF >> /etc/profile.d/elixir-path.sh
export PATH=\$HOME/.elixir-install/installs/otp/${ACTUAL_OTP_VERSION}/bin:\$PATH
export PATH=\$HOME/.elixir-install/installs/elixir/${ACTUAL_ELIXIR_VERSION}/bin:\$PATH
EOF

# Make the profile.d script executable
chmod +x /etc/profile.d/elixir-path.sh

# Create a symbolic link to make it available to all users
mkdir -p /usr/local/bin

if [ ! -f "/usr/local/bin/iex" ]; then
    ln -s "$_REMOTE_USER_HOME/.elixir-install/installs/elixir/${ACTUAL_ELIXIR_VERSION}/bin/iex" /usr/local/bin/iex
    ln -s "$_REMOTE_USER_HOME/.elixir-install/installs/elixir/${ACTUAL_ELIXIR_VERSION}/bin/elixir" /usr/local/bin/elixir
    ln -s "$_REMOTE_USER_HOME/.elixir-install/installs/elixir/${ACTUAL_ELIXIR_VERSION}/bin/mix" /usr/local/bin/mix
fi
